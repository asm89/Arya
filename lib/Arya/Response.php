...
class Response {
    ...
    function __construct($body = NULL, $status = NULL, array $headers = array(), $reason = '') {
        if (isset($body)) {
            $this->setBody($body);
        }
        if (isset($status)) {
     ...
     * @return Response Returns the current object instance
     */
    function setAllHeaders(array $headerArray) {
     ...
     * @return Response Returns the current object instance
     */
    function setCookie($name, $value = '', array $options = array()) {
        $this->assignCookieHeader($name, $value, $options, $rawFlag = FALSE);
     ...
     * @return Response Returns the current object instance
     */
    function setRawCookie($name, $value = '', array $options = array()) {
        $this->assignCookieHeader($name, $value, $options, $rawFlag = TRUE);
    }

    private function assignCookieHeader($name, $value, array $options, $rawFlag) {
        ...
        $header = current(headers_list());
        $value = ltrim(substr($header, 7));
    ...
    /**
     * Retrieve an HTTP status line for this response given the specified HTTP protocol version
     *
     * @param float $protocol
     * @return string
     */
    function generateStatusLine($protocol) {
        $statusLine = sprintf("HTTP/%f %d", $protocol, $this->status);
        if (isset($this->reason[0])) {
            $statusLine .= " {$this->reason}";
        }
